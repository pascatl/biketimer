...
_RSVP_NOTIFICATION_HTML = """<!DOCTYPE html>
<html lang="de">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>RSVP-Benachrichtigung</title>
</head>
<body style="margin:0;padding:20px;background:#f4f4f8;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <table width="480" style="border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(45,60,89,.14);">
          ...
          <tr>
            <td style="background:#fff;padding:32px;">
              <p style="margin:0 0 12px;color:#222;font-size:16px;">Hallo,</p>
              <p style="margin:0 0 20px;color:#444;font-size:15px;line-height:1.6;">
                <strong style="color:#2D3C59;">{actor_name}</strong> {action_text}
              </p>
              <table width="100%" style="border-radius:8px;overflow:hidden;margin-bottom:24px;">
                <tr>
                  <td style="background:{accent_color};height:4px;"></td>
                </tr>
                <tr>
                  <td style="background:#f8f9fc;padding:16px 20px;">
                    <p style="margin:0 0 6px;font-size:13px;font-weight:700;color:#2D3C59;text-transform:uppercase;letter-spacing:1px;">Event-Details</p>
...
def send_rsvp_notification_email(
    ...
    """Send an RSVP notification email (someone accepted or declined). Errors are logged but not raised."""
    if not recipient_email:
        return
    try:
        event_date = event_data.get("event_date", "Unbekannt")
        ...
        if action == "accepted":
            action_text = f"hat zum Event am {event_date} zugesagt."
            subject = f"Zusage zum Event am {event_date}"
            accent_color = "#94A378"
        else:
            action_text = f"hat zum Event am {event_date} abgesagt."
            subject = f"Absage zum Event am {event_date}"
            accent_color = "#D1855C"

        html_body = _RSVP_NOTIFICATION_HTML.format(
        ...
        _log.info(f"RSVP notification mail ({action}) sent to {recipient_email}")
    except Exception as exc:
        _log.error(f"ERROR sending RSVP notification mail to {recipient_email}: {exc}")
